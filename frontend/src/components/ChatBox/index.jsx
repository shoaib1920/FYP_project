...
const ChatBox = () => {
  ...
  useEffect(() => {
    const userData = localStorage.getItem("user") || localStorage.getItem("supervisorData");
  ...
  // Load my department chat(s) — normally just one for a student
  useEffect(() => {
    if (!currentUserId || !token) return;
    ...
    const fetchContacts = async () => {
      try {
        ...
        allTeams.forEach((team) => {
          ...
          const isCreator =
            String(team.createdBy?._id || team.createdBy) === String(currentUserId);
          if (isMember || isCreator) {
            (team.members || []).forEach((m) => memberIds.add(String(m._id || m)));
            if (team.createdBy) memberIds.add(String(team.createdBy._id || team.createdBy));
          }
        });

        const groupMembers = allUsers
          .filter(
            (u) => memberIds.has(String(u._id)) && String(u._id) !== String(currentUserId)
          )
                  ...
                  <div className="wa-contact-info">
                    ...
                    <span className="wa-contact-sub">
                      {group.lastMessage
                        ...
                        : `${group.memberCount} members${group.supervisorName ? ` · Supervisor: ${group.supervisorName}` : ""}`}
                    </span>
                  </div>
            ...
            {filePreview && (
              <div className="file-preview-bar">
                {filePreview.type === "image" && (
                  <img src={filePreview.localUrl} alt="preview" className="fp-img" />
                )}
                {filePreview.type === "video" && (
                  <video src={filePreview.localUrl} className="fp-video" muted />
                )}
                {filePreview.type === "document" && (
                  <span className="fp-doc">📄 {filePreview.name}</span>
                )}
                <button className="fp-remove" onClick={() => setFilePreview(null)}>
                  ✕
                </button>
              </div>
            )}
              ...
              <div className="wa-header-info">
                ...
                <span className="wa-header-status">
                  {groupTypingUsers.size > 0
                    ...
                    : `${selectedGroup.memberCount} members${selectedGroup.supervisorName ? ` · Supervisor: ${selectedGroup.supervisorName}` : ""}`}
                </span>
              </div>
            </div>
            ...
            <div className="wa-messages" ref={messagesContainerRef}>
              {messages.map((msg, i) => {
                ...
                return (
                  <div
                    key={msg._id || i}
                    className={`wa-bubble-wrap ${isMine ? "mine" : "theirs"}`}
                  >
                    <div className={`wa-bubble ${isMine ? "sent" : "received"}`}>
                      {msg.fileUrl && renderMediaInMessage(msg)}
                      {msg.message && <p className="bubble-text">{msg.message}</p>}
